"""Join the back-side CELL_POS sense resistor R104 to the main battery branch."""
...
def add_route(board: pcbnew.BOARD, net: pcbnew.NETINFO_ITEM) -> tuple[int, int]:
    for start, end in zip(BACK_ROUTE, BACK_ROUTE[1:]):
        add_track(board, net, start, end, pcbnew.B_Cu)
    for start, end in zip(FRONT_ROUTE, FRONT_ROUTE[1:]):
        add_track(board, net, start, end, pcbnew.F_Cu)
    add_via(board, net, BACK_ROUTE[-1])
    return len(BACK_ROUTE) + len(FRONT_ROUTE) - 2, 1
...
def main() -> int:
    ...
    sense = pad(board, "R104", "1")
    charger = pad(board, "C104", "1")
    if sense.GetNetname() != NET_NAME or charger.GetNetname() != NET_NAME:
        raise RuntimeError("CELL_POS endpoint assignment changed")

    added_tracks = 0
    added_vias = 0
    if not connected(board, sense, charger):
        net = board.FindNet(NET_NAME)
        if net is None:
            raise RuntimeError(f"Required net is missing: {NET_NAME}")
        added_tracks, added_vias = add_route(board, net)
    ...
    if not connected(reloaded, pad(reloaded, "R104", "1"), pad(reloaded, "C104", "1")):
        raise RuntimeError("R104.1 is still disconnected from the main CELL_POS branch")

    print(
        f"Saved CELL_POS sense candidate: {output_path}; "
