"""Join the back-side CELL_POS monitor input U8.3 to local bypass C120.1."""
...
def main() -> int:
    ...
    monitor = pad(board, "U8", "3")
    bypass = pad(board, "C120", "1")
    if monitor.GetNetname() != NET_NAME or bypass.GetNetname() != NET_NAME:
        raise RuntimeError("CELL_POS endpoint assignment changed")

    added_tracks = 0
    if not connected(board, monitor, bypass):
    ...
    if not connected(reloaded, pad(reloaded, "U8", "3"), pad(reloaded, "C120", "1")):
        raise RuntimeError("U8.3 is still disconnected from C120.1")

    print(
        f"Saved CELL_POS monitor candidate: {output_path}; tracks={added_tracks}",
        flush=True,
    )
    return 0
